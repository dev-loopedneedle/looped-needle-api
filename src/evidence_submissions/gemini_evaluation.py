...
async def evaluate_submission_with_gemini(
    ...
    try:
        ...
        if evidence_claim.category:
            claims.append(
                {
                    "id": _generate_claim_id(claim.id, offset=1000),
                    "name": evidence_claim.category.value
        ...
        gcs_uri = f"gs://{settings.gcs_bucket_name}/{submission.file_path}"

        gemini_client = get_gemini_client()
        evaluation_response = await gemini_client.analyze_document(
            input_file_uri=gcs_uri,
...
async def process_workflow_submissions_concurrently(
    ...
    try:
        ...
        logger.error(
            f"Critical error processing workflow {workflow_id} submissions: {e}",
            exc_info=True,
        )
        try:
            await WorkflowSubmissionService.update_workflow_status_after_processing(db, workflow_id)
            ...
            logger.error(
                f"Failed to update workflow {workflow_id} status after error: {status_error}",
                exc_info=True,
            )
        raise
